...
# Initialize the English word set (lowercase for easier comparison)
ENGLISH_WORDS = set(words.words())

def is_gibberish(word: str, min_length: int = 3) -> bool:
    """
    Determine if a word is gibberish based on length, dictionary presence, and character patterns.
    ...
    # Heuristic: Check for repetitive characters or random patterns
    if len(set(word)) / len(word) < 0.5:  # e.g., "aaa" or "ababab"
        return True
    
    # Additional heuristic: Too many vowels or consonants in a row
    vowels = set('aeiou')
    vowel_count = sum(1 for char in word if char in vowels)
    if vowel_count / len(word) < 0.2 or vowel_count / len(word) > 0.8:  # Unbalanced
        return True
...
def calculate_text_quality(ocr_text: str) -> tuple[float, float]:
    ...
    Returns:
        tuple[float, float]: (percentage of English text, percentage of gibberish text) (0 to 100).
    """
    ...
    # Ensure no overlap (words can be neither English nor gibberish, e.g., short words)
    non_gibberish_non_english = total_words - english_words - gibberish_words
    if non_gibberish_non_english < 0:
        non_gibberish_non_english = 0

    # Calculate percentages
    english_percentage = (english_words / total_words) * 100
    gibberish_percentage = (gibberish_words / total_words) * 100
    remaining_percentage = 100 - english_percentage - gibberish_percentage
